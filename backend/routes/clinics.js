  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (includeGoogle && process.env.GOOGLE_PLACES_API_KEY) {
        try {
          clinics =
            await googlePlacesService.enrichClinicsWithGoogleData(clinics);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingResult.rows.length > 0) {
      throw new AppError(
        "Clinic with this name and address already exists",
        400,
        "CLINIC_EXISTS",
      );
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.user.role !== "super_admin" && clinic.owner_id !== req.user.id) {
      throw new AppError(
        "Not authorized to update this clinic",
        403,
        "NOT_AUTHORIZED",
      );
    }
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.user.role !== "super_admin" && clinic.owner_id !== req.user.id) {
      throw new AppError(
        "Not authorized to delete this clinic",
        403,
        "NOT_AUTHORIZED",
      );
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const clinicResult = await query("SELECT id FROM clinics WHERE id = $1", [
      id,
    ]);
    ...
    if (existingResult.rows.length > 0) {
      throw new AppError(
        "Clinic is already in favorites",
        400,
        "ALREADY_FAVORITED",
      );
    }
