...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      clinicResult = await query("SELECT id, name FROM clinics WHERE id = $1", [
        clinicId,
      ]);
    } else {
    ...
    if (!req.user && (!guestName || !guestEmail)) {
      throw new AppError(
        "Guest name and email are required for guest bookings",
        400,
