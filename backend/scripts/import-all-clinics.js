...
const clinicsDataMatch = scriptContent.match(
  /let clinicsData = (\[[\s\S]*?\]);/,
);
if (!clinicsDataMatch) {
...
function mapClinicData(frontendClinic, index) {
  ...
  const postcodeMatch = frontendClinic.address?.match(
  ...
  return {
    ...
    description:
      frontendClinic.description ||
      `${frontendClinic.name} is a professional healthcare provider in ${city}, offering quality medical services to the local community.`,
  ...
  try {
    ...
    for (let i = 0; i < clinicsData.length; i++) {
      ...
      try {
        ...
        console.log(
          `📝 Processing clinic ${i + 1}/${clinicsData.length}: ${mappedClinic.name}`,
        );
        ...
        importedCount++;
        console.log(
          `✅ Imported: ${mappedClinic.name} (Frontend ID: ${mappedClinic.frontend_id})`,
        );
      } catch (error) {
        console.error(
          `❌ Error importing clinic ${frontendClinic.name || "Unknown"}:`,
          error.message,
        );
        errorCount++;
    ...
    const sampleResult = await client.query(
      "SELECT name, type, city FROM clinics LIMIT 5",
    );
