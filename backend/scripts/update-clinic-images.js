...
async function updateClinicImagesMain() {
    try {
        ...
        try {
            const result = await client.query('SELECT id, name, logo_url FROM clinics ORDER BY name');
            ...
            console.log(`⚠️  About to update ${matches.length} clinic records. Continue? (This will overwrite existing logo_url values)`);
